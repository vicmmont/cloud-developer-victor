...
describe('concat tests', () => {
  it('throws an error if the first argument is an empty string', () => {
    expect(() => { concat('', 'hello')}).to.throw('both strings must be non-empty')
  });

  it('throws an error if the second argument is an empty string', () => {
    expect(() => { concat('world', '')}).to.throw('both strings must be non-empty')
  });
